...
func GetPv(pvName string) (pv *v1.PersistentVolume, err error) {
	config, err := rest.InClusterConfig()
	if err != nil {
		return nil, err
	}
...
func GetCredentialsFromSecret(secretRef *v1.SecretReference) (map[string]string, error) {
	config, err := rest.InClusterConfig()
	if err != nil {
		return nil, err
	}
	...
	secret, err := clientset.CoreV1().Secrets(secretRef.Namespace).Get(secretRef.Name, metav1.GetOptions{})
	if err != nil {
...
func RegisterMount(volumeID string, targetPath string, node string, options map[string]string) (err error) {
	config, err := rest.InClusterConfig()
	if err != nil {
		return err
	}
	...
	_, err = clientset.GcsV1beta1().PublishedVolumes().Create(&v1beta1.PublishedVolume{
		...
		Status: v1beta1.PublishedVolumeStatus{
			Name: "PENDING",
		},
	})
	if err != nil {
...
func UpdateMountStatus(volumeID string, targetPath string, node string, status string) (err error) {
	config, err := rest.InClusterConfig()
	if err != nil {
		return err
	}
	...
	publishedVolume, err := clientset.GcsV1beta1().PublishedVolumes().Get(name, metav1.GetOptions{})
	if err != nil {
		return err

	}

	publishedVolume.Status.Name = status
	_, err = clientset.GcsV1beta1().PublishedVolumes().Update(publishedVolume)
	if err != nil {
		return err
	}
